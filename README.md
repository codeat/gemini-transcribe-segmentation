...
2. **请求参数**：`language_codes` 是否配置按语言与音频长度 A/B 实测后决定（本次中文整文件传入时触发 272 秒单段）；`diarization` 与 `word_timestamp` 保持同开（关词级时间戳后最长段反弹到 745 字）。
...
结论：不同语言是否配置 `language_codes`，需要根据实际场景和音频文件长度多做测试后按实践结果决定；本次中文 24 分钟整文件传入语言提示时出现 272 秒单段，分块后差异明显缩小。**按 10 分钟分块**的收益最大：切换 28 → 52，标签一致率 65% → 90%。所有配置均开启说话人分离与词级时间戳。
